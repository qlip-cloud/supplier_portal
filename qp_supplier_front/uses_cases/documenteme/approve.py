...
def get_registrable_warnings(doc, po_exists_fn, receipt_bank_fn, epsilon=DEFAULT_EPSILON):
    ...
    Las facturas de CONTADO relajan OC y recibos/montos, por lo que sus
    advertencias son generadas solo si llegan a la parte de credito.
    """
...
def validate_registrable(doc, po_exists_fn, receipt_bank_fn, epsilon=DEFAULT_EPSILON):
    ...
    warnings = get_registrable_warnings(doc, po_exists_fn, receipt_bank_fn, epsilon)
...
def collect_registrable_violations(docs, po_exists_fn, receipt_bank_fn, epsilon=DEFAULT_EPSILON):
...
def validate_registrables(docs, po_exists_fn, receipt_bank_fn, epsilon=DEFAULT_EPSILON):
    valid = []
    errors = []
    for doc in (docs or []):
        ok, error = validate_registrable(doc, po_exists_fn, receipt_bank_fn, epsilon)
        if ok:
            valid.append(doc)
        else:
            errors.append({
...
def _allocate_registrables(docs, po_exists_fn, receipt_bank_fn, epsilon):
    ...
    for purchase_order in credit_order:
        ...
        for doc in group:
            ...
            if matched is None:
                errors.append({
                    "nvfac_nume": doc.get("nvfac_nume"),
                    "error": _no_match_warning(doc.get("nvfac_totp") or 0),
                })
            else:
...
def approve_documents(
    ...
    if force:
        ...
        valid, errors = validate_registrables(
            docs, po_exists_fn, _scalar_bank_fn(receipts_total_fn), epsilon
        )
        allocation = {}
