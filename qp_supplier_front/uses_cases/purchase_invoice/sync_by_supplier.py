...
def _run_global_gp(mode):
    """
    Flujo automatico GP: la estrategia GP no tiene sincronizacion global por
    ventanas (solo por proveedor). Se itera todos los proveedores con el
    mismo flujo per-supplier del portal, respetando las fechas del checkout
    por proveedor y el modo (full/incremental).
    """
    ...
    for supplier_id in suppliers:
        try:
            ...
            log_error(
                message=str(e),
                title="Error sync all purchase invoices (GP): {}".format(supplier_id),
            )
    return {
        "inserted": total_inserted,
        "windows": 0,
...
@frappe.whitelist()
def sync_all_suppliers(flow=None):
